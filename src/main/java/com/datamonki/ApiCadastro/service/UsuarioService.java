...
public class UsuarioService {
	...
	private void verificar(UsuarioDto usuarioDto) {
		List<String> messages = new ArrayList<>();

		if (usuarioDto.userName().isBlank()) {
			messages.add("O campo userName está vazio");
		} else if (usuarioDto.userName().length() < 3 || usuarioDto.userName().length() > 50) {
			messages.add("O userName deve ter entre 3 e 50 caracteres");
		}

		if (usuarioDto.password().isBlank()) {
			messages.add("O campo password está vazio");
		} else if (usuarioDto.password().length() < 5) {
			messages.add("A senha precisa ter mais de 5 caracteres");
		}
